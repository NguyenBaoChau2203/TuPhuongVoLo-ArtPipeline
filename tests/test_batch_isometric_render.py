"""Tests for Feature 004 batch isometric rendering without Blender."""
...
import batch_isometric_render as batch
...
def isolated_manifest(monkeypatch, tmp_path: Path) -> Path:
    """Redirect Feature 001 manifest lookups to a temp path."""
...
def test_input_file_creates_one_job(tmp_path: Path, monkeypatch) -> None:
    isolated_manifest(monkeypatch, tmp_path)
...
def test_room_filter_selects_normalized_vietnamese_room_name(tmp_path: Path, monkeypatch) -> None:
    isolated_manifest(monkeypatch, tmp_path)
    svg = write_svg(
        tmp_path / "rooms.svg",
        """
...
def test_corrupt_svg_is_failed_but_batch_continues_by_default(tmp_path: Path, monkeypatch) -> None:
    isolated_manifest(monkeypatch, tmp_path)
    input_dir = tmp_path / "svg_clean"
    input_dir.mkdir()
    bad_svg = input_dir / "bad.svg"
...
def test_job_file_can_be_loaded_and_dry_run_processed(tmp_path: Path, monkeypatch) -> None:
    isolated_manifest(monkeypatch, tmp_path)
    svg = write_svg(tmp_path / "one.svg")
                ...
                "jobs": [
                    {
                        ...
                        "room_preset": "kho",
                        "planned_blend_output": str(tmp_path / "outputs" / "blender" / "x.blend"),
                        "planned_preview_output": str(tmp_path / "outputs" / "preview" / "x.png"),
                    }
                ]
            }
        ),
...
def test_planned_outputs_follow_naming_convention(tmp_path: Path, monkeypatch) -> None:
    isolated_manifest(monkeypatch, tmp_path)
    svg = write_svg(tmp_path / "one.svg")
    ...
    assert job.planned_blend_output is not None
    assert job.planned_preview_output is not None
    assert job.planned_blend_output.name == "tu_phuong_vo_lo_kho_main_iso_v001.blend"
    assert job.planned_preview_output.name == "tu_phuong_vo_lo_kho_main_preview_v001.png"


def test_feature001_command_includes_build_isometric_room(tmp_path: Path, monkeypatch) -> None:
    isolated_manifest(monkeypatch, tmp_path)
    svg = write_svg(tmp_path / "one.svg")
    args = parser_args(
        [
    ...
    command = batch.build_feature001_command(report.jobs[0], dry_run=True)

    assert any("build_isometric_room.py" in part for part in command)
    assert "--dry-run" in command


def test_actual_render_missing_blender_fails_gracefully(tmp_path: Path, monkeypatch) -> None:
    isolated_manifest(monkeypatch, tmp_path)
    svg = write_svg(tmp_path / "one.svg")
    ...
    assert "Blender" in payload["jobs"][0]["message"]
